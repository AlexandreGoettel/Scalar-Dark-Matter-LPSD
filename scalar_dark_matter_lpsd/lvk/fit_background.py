...
def calc_bic(x, y, n_knots, bin_scaling=.15, buffer=40, cfd_fraction=.1):
    ...
    def log_lkl(params):
        ...
        lkl = np.full(pdf.shape, -sys.float_info.max, dtype=np.float64)
        mask = pdf == 0
    ...
    # total_popt = minimize(log_lkl, p0, method="L-BFGS-B", bounds=bounds)
...
def process_iteration(args):
    ...
    if verbose:
        ...
        axData.plot(np.exp(x), y, ".", label="Data", zorder=1)
        k = len(func_popt) // 2
        axData.plot(np.exp(x), y_model, linewidth=2.5, zorder=3, label=f"Best fit (k: {k})")
        axData.scatter(np.exp(func_popt[:k]), func_popt[k:], color="r", zorder=4, label="Spline knots", s=64)
        if plot_mean:
            axData.plot(np.exp(x), stats.kde_smoothing(y, kernel_size), label="kde", zorder=2)
        axData.legend(loc="best")
        axData.set_ylabel("logPSD")
        axData.set_xlabel("Frequency (Hz)")
        axData.grid(color="grey", alpha=.7, linestyle="--")
        ...
        axHist.set_title(r"$\chi^2/dof$: " + f"{chi_sqr / (np.sum(pos_h0) - 3):.1f}")
